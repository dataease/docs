...
!!! Abstract ""
    ...
    若是离线安装方式，在 DataEase 离线安装包解压目录 dataease-v1.12.0-offline/ 下，重新执行安装脚本即可。  
    **注意：由于精简模式的 Excel 与 API 数据集是保存在配置的数据引擎中的，切换为其它模式会导致这两部分的数据丢失，反之亦然（Excel 与 API 数据集保存位置变更），直连数据库的相关数据集不受影响。**  
    ```shell
    bash install.sh
    ```
    若是在线安装方式，在执行一键安装命令的在线安装包解压目录 dataease-v1.12.0-online/ 下，重新执行 bash install.sh，也可重新执行在线安装脚本。
    ```
