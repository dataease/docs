...
!!! Abstract ""	
	公共链接参数组装对传参进行 Base64 处理，以下为示例：配置参数名为"省份"，关联了数据大屏中【省份】文本下拉过滤组件（也可关联对应的图表字段），组装参数为 {"省份":"北京市"}，使用 base64 加密处理，该示例通过电脑终端进行，其它方式均可。
	```
	echo '{"省份":"北京市"}'|base64
	```
...
![数据大屏_批量操作](../img/dashboard_generation/外部传参示例.png){ width="900px" }
...
![数据大屏_关闭](../img/panel_generation/2.0数据大屏返回.png){ width="900px" }
